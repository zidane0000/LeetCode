...
func findRelativeRanks(scores []int) []string {
	// create a copy score and sort it by descending
	SortedScores := make([]int, len(scores))
	copy(SortedScores, scores)
	sort.Slice(SortedScores, func(a, b int) bool {
		return SortedScores[a] > SortedScores[b]
	})

	// create a map represent score map to rand by Sorted Scores
	ScoreMapRank := make(map[int]string)
	for i, score := range SortedScores {
		if i == 0 {
			ScoreMapRank[score] = "Gold Medal"
		} else if i == 1 {
			ScoreMapRank[score] = "Silver Medal"
		} else if i == 2 {
			ScoreMapRank[score] = "Bronze Medal"
		} else {
			ScoreMapRank[score] = strconv.Itoa(i + 1)
		}
	}

	// create ans based on ScoreMapRank
	var ans []string
	for _, score := range scores {
		ans = append(ans, ScoreMapRank[score])
	}
	return ans
}
